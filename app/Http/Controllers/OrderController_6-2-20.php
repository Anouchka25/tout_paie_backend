		...
		try{
			...
			foreach ($getProductToDeliver as $key => $value) {
				$store =DB::table('tblStore')
				   ...
				   ->get();    
				   $totalDeliveryAmount = floatval($totalDeliveryAmount) + floatval($store[0]->DeliveryPrice);
			}
				...
				try {
					...
					try {
						...
						foreach ($orderProductsData as $key => $value) {
							...
							$user= $user->select('Email','FirstName','LastName','MobileNumber','City','DeviceID','DeviceType','StoreID')->where('StoreID', $value->StoreID)->first();
							...
							$FirstName = (string)$user['FirstName'];
				      		...
				      		$deliveryPrice = (string)$storeDeliveryPrice['DeliveryPrice'];

				      		$data = array("FirstName" => $user['FirstName'],"LastName" => $user['LastName'],"MobileNumber" => $user['MobileNumber'], "Email" => $user['Email'], 'OrderID' =>$orderNumber,'OrderDate' =>$orderDate,"City" => $user['City'],"ResidenceStreetName" => $StreetName,"ResidencePostalCode" => $PostalCode,"ResidenceCity" => $City,'OrderItems' => $trader , "TotalDeliveryAmount" => $deliveryPrice);
							...
							if($userEmail !=''){
								//Email to trader
					      		...
					      		{			
						        	...
						        	->subject('Order Email');
						        	$message->from('mindnervesdemo@gmail.com','Tout Paie Team');
							    });
							}
